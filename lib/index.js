 ...
 * - syncs the packaged skill.md into ~/.dsh/skills/kimi-webbridge/SKILL.md,
 *   the model-facing skill that documents the full 17-action HTTP protocol
 *   for driving the user's real browser;
...
const Config = z.object({
  ...
  generateSkill: z.boolean().default(true)
});
...
/** Directory the DSH skill loader scans for the generated skill. */
function skillDir() {
  return join(homedir(), ".dsh", "skills", "kimi-webbridge");
}

/**
 * Sync the packaged skill.md into the DSH skills directory so the model can
 * load it with the skill tool. Rewrites only when missing or changed, so
 * restarts stay cheap and updates propagate automatically.
...
async function syncSkill() {
  try {
    ...
    // Never break plugin startup over skill syncing; the health tool still works.
    console.warn(`kimi-webbridge: skill sync failed: ${error?.message ?? error}`);
    return false;
  }
}
...
function apply(ctx, config = {}) {
  ...
  if (generateSkill) void syncSkill();
