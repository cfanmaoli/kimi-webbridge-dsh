...
async function installDaemonBinary({ version, timeoutMs }) {
  ...
  const url = `${CDN_BASE}/${encodeURIComponent(version)}/releases/kimi-webbridge-${platform}`;
      ...
      try {
        ...
        if (!existsSync(daemonBinPath())) {
          if (!autoInstallDaemon) {
            throw new Error(`kimi-webbridge: daemon binary not found at ${daemonBinPath()} and autoInstallDaemon is disabled. Install it with: irm https://cdn.kimi.com/webbridge/install.ps1 | iex`);
          }
          try {
            const { url } = await installDaemonBinary({ version: daemonVersion, timeoutMs: requestTimeoutMs });
            notes.push(`installed daemon from ${url}`);
          } catch (installError) {
            throw new Error(`kimi-webbridge: daemon binary missing and auto-install failed: ${installError?.message ?? installError}. Install it manually: irm https://cdn.kimi.com/webbridge/install.ps1 | iex`);
          }
        }
        if (autoStartDaemon) {
          ...
          try {
            ...
            throw new Error(`kimi-webbridge: daemon at ${baseUrl} still unreachable after starting it.${hint} Check ~/.kimi-webbridge/logs/daemon.log or reinstall: irm https://cdn.kimi.com/webbridge/install.ps1 | iex`);
          }
        } else {
